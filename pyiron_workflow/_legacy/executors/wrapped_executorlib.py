...
from pyiron_workflow._legacy.mixin import lexical, run
    ...
    def submit(self, fn, /, *args, **kwargs):
        ...
        if (
            ...
            raise DedicatedExecutorError(
                ...
                f"on_run method of pyiron_workflow._legacy.Node objects, but got {fn}"
            )
    ...
    if "resource_dict" in kwargs:
        if "cache_key" in kwargs["resource_dict"]:
            raise ProtectedResourceError(
                f"pyiron_workflow._legacy needs the freedom to specify the cache, so the "
