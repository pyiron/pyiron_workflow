...
To accomplish this, we overload :class:`pyiron_workflow._legacy.channel.OutputData` to be able
...
from pyiron_workflow._legacy.channels import OutputData
from pyiron_workflow._legacy.data import NOT_DATA
from pyiron_workflow._legacy.io import GenericOutputs
from pyiron_workflow._legacy.mixin.has_interface_mixins import HasChannel
...
class OutputDataWithInjection(OutputData):
    """
    Output data that must have a :class:`pyiron_workflow._legacy.node.Node` for its
    ...
    def __getattr__(self, name):
        from pyiron_workflow._legacy.nodes.standard import GetAttr  # noqa: PLC0415
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, slice) and any(
            ...
            from pyiron_workflow._legacy.nodes.standard import Slice  # noqa: PLC0415

            item = self._node_injection(
                Slice, item.start, item.stop, item.step, inject_self=False
            )

        from pyiron_workflow._legacy.nodes.standard import GetItem  # noqa: PLC0415

        return self._node_injection(GetItem, item)

    def __lt__(self, other):
        from pyiron_workflow._legacy.nodes.standard import LessThan  # noqa: PLC0415

        return self._node_injection(LessThan, other)

    def __le__(self, other):
        from pyiron_workflow._legacy.nodes.standard import (  # noqa: PLC0415
            LessThanEquals,
        )
    ...
    def eq(self, other):
        from pyiron_workflow._legacy.nodes.standard import Equals  # noqa: PLC0415

        return self._node_injection(Equals, other)

    def __ne__(self, other):
        from pyiron_workflow._legacy.nodes.standard import NotEquals  # noqa: PLC0415

        return self._node_injection(NotEquals, other)

    def __gt__(self, other):
        from pyiron_workflow._legacy.nodes.standard import GreaterThan  # noqa: PLC0415
    ...
    def bool(self):
        from pyiron_workflow._legacy.nodes.standard import Bool  # noqa: PLC0415

        return self._node_injection(Bool)

    def len(self):
        from pyiron_workflow._legacy.nodes.standard import Length  # noqa: PLC0415

        return self._node_injection(Length)

    def contains(self, other):
        from pyiron_workflow._legacy.nodes.standard import Contains  # noqa: PLC0415

        return self._node_injection(Contains, other)

    def __add__(self, other):
        from pyiron_workflow._legacy.nodes.standard import Add  # noqa: PLC0415

        return self._node_injection(Add, other)

    def __sub__(self, other):
        from pyiron_workflow._legacy.nodes.standard import Subtract  # noqa: PLC0415

        return self._node_injection(Subtract, other)

    def __mul__(self, other):
        from pyiron_workflow._legacy.nodes.standard import Multiply  # noqa: PLC0415
    ...
    def __truediv__(self, other):
        from pyiron_workflow._legacy.nodes.standard import Divide  # noqa: PLC0415

        return self._node_injection(Divide, other)

    def __floordiv__(self, other):
        from pyiron_workflow._legacy.nodes.standard import FloorDivide  # noqa: PLC0415

        return self._node_injection(FloorDivide, other)

    def __mod__(self, other):
        from pyiron_workflow._legacy.nodes.standard import Modulo  # noqa: PLC0415

        return self._node_injection(Modulo, other)

    def __pow__(self, other):
        from pyiron_workflow._legacy.nodes.standard import Power  # noqa: PLC0415

        return self._node_injection(Power, other)

    def __and__(self, other):
        from pyiron_workflow._legacy.nodes.standard import And  # noqa: PLC0415

        return self._node_injection(And, other)

    def __xor__(self, other):
        from pyiron_workflow._legacy.nodes.standard import XOr  # noqa: PLC0415

        return self._node_injection(XOr, other)

    def __or__(self, other):
        from pyiron_workflow._legacy.nodes.standard import Or  # noqa: PLC0415

        return self._node_injection(Or, other)

    def __neg__(self):
        from pyiron_workflow._legacy.nodes.standard import Negative  # noqa: PLC0415

        return self._node_injection(Negative)

    def __pos__(self):
        from pyiron_workflow._legacy.nodes.standard import Positive  # noqa: PLC0415

        return self._node_injection(Positive)

    def __abs__(self):
        from pyiron_workflow._legacy.nodes.standard import Absolute  # noqa: PLC0415

        return self._node_injection(Absolute)

    def __invert__(self):
        from pyiron_workflow._legacy.nodes.standard import Invert  # noqa: PLC0415

        return self._node_injection(Invert)

    def int(self):
        from pyiron_workflow._legacy.nodes.standard import Int  # noqa: PLC0415

        return self._node_injection(Int)

    def float(self):
        from pyiron_workflow._legacy.nodes.standard import Float  # noqa: PLC0415

        return self._node_injection(Float)

    def __round__(self):
        from pyiron_workflow._legacy.nodes.standard import Round  # noqa: PLC0415
