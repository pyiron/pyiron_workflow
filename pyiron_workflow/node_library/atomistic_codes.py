...
def Lammps(wf: Macro, structure, potential_name: Optional[str] = None):
    ...
    wf.init_lammps.inputs.working_directory = (
        wf.init_lammps.working_directory.path.__str__()
    )
    ...
    wf.parser_dump_file = wf.create.lammps.ParserDumpFile(
        dump_file=wf.shell.outputs.dump
    )
    wf.collect = wf.create.lammps.Collect(
        out_dump=wf.parser_dump_file.outputs.dump,
        out_log=wf.parser_log_file.outputs.log,
    )
    return wf.collect
