    ...
    def test_valid_data_nested_and_zipped(self):
        ...
        expected_maps = tuple(
            ...
            for z_idx, z_idx2 in zip(
                range(len(data["zipped1"])), range(len(data["zipped2"])), strict=False
            )
        )
        self.assertEqual(
            expected_maps,
...
class TestForNode(unittest.TestCase):
    ...
    def test_with_connections_list(self):
        ...
        self.assertEqual(
            x[0] + y, li_out[0], msg="Just make sure the loop is actually running"
        )
        x, y = [2, 3], 4
        li_out = li(x, y).out
        self.assertEqual(length_y * len(x), len(li_out))
